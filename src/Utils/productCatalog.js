...
export const PRODUCT_BRANDS = {
    ...
    4: {
        ...
        logo: '/products/processhub.jpg',
        innerBg: '#ffffff',
    },
    5: {
        color: '#EA580C',
        gradient: 'linear-gradient(135deg, #C2410C 0%, #EA580C 45%, #FB923C 100%)',
        logo: '/products/ape.png',
        innerBg: '#ffffff',
        free: true,
    },
};
...
export function getProductCatalog(translations) {
    ...
    return items.map((item, index) => ({
        ...
        isPeopleHub: index === 2,
        isFree: Boolean(PRODUCT_BRANDS[index]?.free || item.free),
    }));
}

export function getSolutionsProducts(translations) {
    return getProductCatalog(translations).slice(0, 6);
}

export function getMoreSolutionsProducts(translations) {
    return getProductCatalog(translations).slice(6);
}
